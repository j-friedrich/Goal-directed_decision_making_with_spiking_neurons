...
def run(np.ndarray[DTYPE_t, ndim=2] W, double step, int rate=100,
    ...
    cdef int t
    cdef int k
    cdef double cr  # constant current into reward neuron
    ...
    if reset == 0:
        for t in range(1, int(T / step)):
            ...
            u[t, K - 1] = rate / 1000.
            for k in range(K):
                spikes[t, k] = rand() < step * u[t, k] * RAND_MAX
    else:
        u[0, K - 1] = rate / 1000.
        f *= (1 + reset)
        ...
        for t in range(1, int(T / step)):
            ...
            if 1 in spikes[t - 1]:
                c += np.dot(W, spikes[t - 1]) * f
            u[t] = u[t - 1] * fm + c - f2 * spikes[t - 1]
            u[t, K - 1] += cr
            for k in range(K):
                spikes[t, k] = rand() < step * u[t, k] * RAND_MAX
    return np.array([spikes, u])
